...
if __name__ == "__main__":
  ...
  os.makedirs("sbsg_grid_search", exist_ok=True)
  os.chdir('./sbsg_grid_search')
  save_dir = os.getcwd()
  ...
  '''
  # 5-way grid search
  for w_pm_1 in ["1e2", "1e3", "1e4"]:
    for w_pm_2 in ["1e1", "1e2", "1e3"]:
      for w_pm_3 in ["1e6", "1e7", "1e8"]:
        for w_pm_4 in ["0.1", "1", "10"]:
          for w_pm_5 in ["0.5", "5", "50"]:
            current_lua = copy.deepcopy(base_lua)
            current_lua.append("POSE_GRAPH.optimization_problem.odometry_translation_weight = "         + w_pm_1 + "\n")
            current_lua.append("POSE_GRAPH.constraint_builder.loop_closure_translation_weight = "       + w_pm_2 + "\n")
            current_lua.append("POSE_GRAPH.optimization_problem.local_slam_pose_translation_weight = "  + w_pm_3 + "\n")
            current_lua.append("POSE_GRAPH.constraint_builder.ceres_scan_matcher.translation_weight = " + w_pm_4 + "\n")
            current_lua.append("POSE_GRAPH.matcher_translation_weight = "                               + w_pm_5 + "\n")
